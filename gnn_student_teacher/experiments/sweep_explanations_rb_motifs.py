...
SHORT_DESCRIPTION = (
    'Executes a student teacher sweep over different explanations for the rbmotifs dataset'
)

# == DATASET PARAMETERS ==
DATASET_PATH = os.path.join(DATASETS_PATH, 'rb_dual_motifs')
METADATA_CONTAINS_INDEX = True
METADATA_CONTAINS_SPLIT = True
METADATA_TARGET_KEY = 'value'
IS_REGRESSION = True
TEST_SPLIT = 0.2
NODE_IMPORTANCES_KEY = 'multi_node_importances'
EDGE_IMPORTANCES_KEY = 'multi_edge_importances'
...
SWEEP = {
    ...
    'megan': {
        'NODE_IMPORTANCES_KEY': 'node_importances_megan',
        'EDGE_IMPORTANCES_KEY': 'edge_importances_megan',
    }
}
...
NAMESPACE = 'sweep/explanations_rb_motifs'
DEBUG = True
with Skippable(), (e := Experiment(base_path=os.getcwd(), namespace=NAMESPACE, glob=globals())):
    ...
    for sweep_name, sweep_params in SWEEP.items():
        ...
        # We are doing this here for every sweep anew because the sweep most likely includes a change
        # of the dataset for this experiment
        ...
        for name, data in dataset_map.items():
            ...
            # The dataset will most likely contain multiple different importance annotations, the concrete
            # ones to be loaded in this iteration are determined by the global variables (subject to change
            # during the sweep!)
            ...
            # If this flag is true we assume that the canonical index of an element within the dataset is
            # saved as part of its metadata and we will use that index then.
            # Otherwise, we will use the index as the files appeared in the dataset folder
            # (os specific sorting!)
        ...
        for rep in range(REPETITIONS):
            ...
            current_train_indices = random.sample(train_indices, k=STUDENT_SIZE)
            # current_train_indices = student_indices
