...
either only the edges or only the node importance explanations are used by the explanation student.
...
with Skippable(), (se := SubExperiment(EXPERIMENT_PATH, BASE_PATH, NAMESPACE, globals())):
    ...
    def sweep_generator(e, dataset):
        """
        In the iterations of this sweep the explanation student either only uses the edge importances for the
        explanation supervision or it only uses the node importances.
        """
        # In this first step we only use the node importances for the explanation supervision
        e.glob['VARIANT_KWARGS'].update({
            'exp': {
                'loss_cb': lambda: [ks.losses.CategoricalCrossentropy(from_logits=True),
                                    ExplanationLoss(),
                                    NoLoss()],
        ...
        # In this second step we only use the edge importances for the explanation supervision
        e.glob['VARIANT_KWARGS'].update({
            'exp': {
                'loss_cb': lambda: [ks.losses.CategoricalCrossentropy(from_logits=True),
                                    NoLoss(),
        ...
        # And then finally both of them as the baseline
        e.glob['VARIANT_KWARGS'].update({
            'exp': {
                'loss_cb': lambda: [ks.losses.CategoricalCrossentropy(from_logits=True),
                                    ExplanationLoss(),
