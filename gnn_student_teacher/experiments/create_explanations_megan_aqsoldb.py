...
# == DATASET PARAMETERS ==
DATASET_PATH = os.path.join(DATASETS_PATH, 'aqsoldb')
...
STUDENT_KWARGS = {
    ...
    'importance_multiplier': 12.0,
    'regression_reference': -2,
    'regression_limits': (-12, 2)
}
...
NAMESPACE = 'create_explanations/megan_aqsoldb'
DEBUG = True
with Skippable(), (e := Experiment(base_path=os.getcwd(), namespace=NAMESPACE, glob=globals())):
    ...
    for name, data in dataset_map.items():
        ...
        # Some datasets may also not have any node_importance annotation at all. In this case we just create
        # mock tensors consisting of all zeros, because at least the existence of these fields is expected
        # in the further program, even if nothing is actively done with them
        if 'node_importances' not in g:
            g['node_importances'] = np.zeros(shape=(len(g['node_attributes'], NUM_IMPORTANCE_CHANNELS)))
            g['edge_importances'] = np.zeros(shape=(len(g['edge_attributes'], NUM_IMPORTANCE_CHANNELS)))
    ...
    if METADATA_CONTAINS_SPLIT:
        ...
        #                 for data in dataset_map.values()
        #                 if data['metadata']['type'] == 'test']
        # train_indices = [data['index']
        #                  for data in dataset_map.values()
        #                  if data['metadata']['type'] == 'train']
    ...
    example_indices = random.sample(student_indices, k=EXAMPLE_SIZE)
    ...
    for rep in range(REPETITIONS):
        ...
        hyper_kwargs = resolve_callbacks_nested_dict(HYPER_KWARGS)
        result = trainer.fit(**{
            **hyper_kwargs,
        ...
        with PdfPages(pdf_path) as pdf:
            ...
            ax.plot([lower, upper], [lower, upper], color='black', alpha=0.5)
            pdf.savefig(fig)
            plt.close(fig)
...
with Skippable(), e.analysis:
    ...
    for index in e['test_indices'] + e['train_indices'] + e['student_indices']:
        ...
        ni = la.norm(nis, axis=0, ord=NORM_ORDER) / REPETITIONS**NORM_ORDER
        ei = la.norm(eis, axis=0, ord=NORM_ORDER) / REPETITIONS**NORM_ORDER
    ...
    with PdfPages(pdf_path) as pdf:
        for c, index in enumerate(e['example_indices']):
            g = dataset[index]
            ...
            if c % EVAL_LOG_STEP == 0:
                e.info(f' * drawn ({c}/{len(e["example_indices"])})')
